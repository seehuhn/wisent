...
def main():
    """Main entry point for the wisent command line tool."""
    ######################################################################
...
information about these matters, see the file named COPYING."""%VERSION)
    ...
    # Handle both bytes (from file) and str (from stdin) in Python 3
    if isinstance(text, bytes):
        text = text.decode("utf-8")
    a = read_grammar(text.splitlines(), params, check)
    del text
