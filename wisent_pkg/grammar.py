...
class Grammar(object):
    ...
    def write_example(self, fd=sys.stdout, params={}):
        ...
        # Add missing template parameters with defaults
        params.setdefault('example_name', 'example')
        params.setdefault('date', time.strftime('%Y-%m-%d %H:%M:%S'))
        params.setdefault('version', VERSION)
        # Don't set a default for 'fname' to preserve conditional logic below

        write_block(fd, 0, """
        #! /usr/bin/env python
        # %(example_name)s - test program with sample input for the generated parser
